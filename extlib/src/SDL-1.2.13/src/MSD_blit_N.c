...
static void Blit_RGB888_index8(MSD_BlitInfo *info)
{
	int width, height;
	uint32_t *src;
	const uint8_t *map;
...
static void Blit_RGB888_RGB555(MSD_BlitInfo *info)
{
	int width, height;
	uint32_t *src;
	uint16_t *dst;
...
static void Blit_RGB888_RGB565(MSD_BlitInfo *info)
{
	int width, height;
	uint32_t *src;
	uint16_t *dst;
...
static void Blit_RGB565_32(MSD_BlitInfo *info, const uint32_t *map)
{
	int width, height;
	uint8_t *src;
	uint32_t *dst;
...
static void Blit_RGB565_ARGB8888(MSD_BlitInfo *info)
{
...
static void Blit_RGB565_ABGR8888(MSD_BlitInfo *info)
{
...
static void Blit_RGB565_RGBA8888(MSD_BlitInfo *info)
{
...
static void Blit_RGB565_BGRA8888(MSD_BlitInfo *info)
{
    Blit_RGB565_32(info, RGB565_BGRA8888_LUT);
}

static void Blit_RGB888_index8_map(MSD_BlitInfo *info)
{
	int Pixel;
	int width, height;
	uint32_t *src;
...
static void BlitNto1(MSD_BlitInfo *info)
{
	int width, height;
	uint8_t *src;
	const uint8_t *map;
...
static void Blit4to4MaskAlpha(MSD_BlitInfo *info)
{
...
static void BlitNtoN(MSD_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
...
static void BlitNtoNCopyAlpha(MSD_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
	uint8_t *src = info->s_pixels;
...
static void BlitNto1Key(MSD_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
	uint8_t *src = info->s_pixels;
...
static void Blit2to2Key(MSD_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
...
static void BlitNtoNKey(MSD_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
	uint8_t *src = info->s_pixels;
...
static void BlitNtoNKeyCopyAlpha(MSD_BlitInfo *info)
{
	int width = info->d_width;
	int height = info->d_height;
	uint8_t *src = info->s_pixels;
...
static const struct MSD_blit_table normal_blit_1[] = {
	{ 0,0,0, 0, 0,0,0, 0, NULL, NULL },
};

static const struct MSD_blit_table normal_blit_2[] = {
...
static const struct MSD_blit_table normal_blit_3[] = {
    { 0,0,0, 0, 0,0,0, 0, NULL, BlitNtoN, 0 }
};

static const struct MSD_blit_table normal_blit_4[] = {
	{ 0,0,0, 0, 0,0,0, 0, NULL, BlitNtoN, 0 }
};

static const struct MSD_blit_table *normal_blit[] = {
	normal_blit_1, normal_blit_2, normal_blit_3, normal_blit_4
};

#define MASKOK(x, y) (((x) == (y)) || ((y) == 0x00000000))

MSD_loblit MSD_CalculateBlitN(MSD_Surface *surface, int blit_index)
{
	struct MSD_private_swaccel *sdata;
	MSD_PixelFormat *srcfmt;
	MSD_PixelFormat *dstfmt;
	const struct MSD_blit_table *table;
	int which;
	MSD_loblit blitfun;
	...
	if (blit_index & 2) {
	    return MSD_CalculateAlphaBlit(surface, blit_index);
	}
	...
	if (blit_index == 1) {
	    if (srcfmt->BytesPerPixel == 2
			...
			return (MSD_loblit)Blit2to2Key;
		} else if(dstfmt->BytesPerPixel == 1) {
			return (MSD_loblit)BlitNto1Key;
		} else {
			if (srcfmt->Amask && dstfmt->Amask) {
				return (MSD_loblit)BlitNtoNKeyCopyAlpha;
			} else {
				return (MSD_loblit)BlitNtoNKey;
			}
	    }
	}
